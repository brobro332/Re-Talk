...
class AndroidParserStrategy : ChatParserStrategy {
    private val messagePattern = """^(\d{4}년\s*\d{1,2}월\s*\d{1,2}일\s*오[전|후]\s*\d{1,2}:\d{2}),\s*(.*?)\s*:\s*(.*)$""".toRegex()
    private val dateHeaderPattern = """^\d{4}년\s*\d{1,2}월\s*\d{1,2}일\s*오[전|후]\s*\d{1,2}:\d{2}$""".toRegex()

    override fun isSupport(line: String): Boolean {
        return line.contains("년") && line.contains("월") && line.contains("일") && (line.contains("오전") || line.contains("오후"))
    }
